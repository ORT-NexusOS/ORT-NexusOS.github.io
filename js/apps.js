...
const Apps = (() => {
    ...
    function gallery() {
      ...
      <div id="lightbox" class="hidden">
        ...
        <button class="btn" onclick="document.getElementById('lightbox').classList.add('hidden')" style="margin-top:16px;">[ FECHAR ]</button>
      </div>`;
    }
    ...
    function missions() {
      ...
      <div id="missions-add-form" class="hidden" style="background:var(--bg-panel);border:1px solid var(--border-dim);padding:16px;margin-bottom:16px;">
        <div style="display:grid;gap:10px;margin-bottom:12px;">
          ...
          <div class="login-field"><label class="login-label">&gt; DESCRIÇÃO</label><input type="text" id="m-desc" placeholder="Briefing da missão..."></div>
        </div>
        <div style="display:flex;gap:10px;">
    ...
    async function saveMission() {
        ...
        const db = Auth.db();
        if (db) await db.from('missions').insert({ title: code, description: desc, status: 'ativa' });
    ...
    async function loadEmails() {
      ...
      <div id="email-body-${e.id}" class="email-body-view hidden">${e.body}</div>`).join('');
    }
    ...
    function composeEmail() {
        const subject = prompt('ASSUNTO DA MENSAGEM:');
        const body = prompt('CORPO DA MENSAGEM:');
        if (!subject) return;
        const db = Auth.db();
        if (db) {
            db.from('emails').insert({ sender: Auth.getProfile()?.display_name || 'ADM', subject, body, recipient: 'all', unread: true })
                .then(() => loadEmails());
        }
    }
    ...
    function notepad() {
      ...
      <div style="display:flex;flex-direction:column;height:100%;gap:8px;">
        <div class="app-toolbar">
          <button class="btn" id="btn-save-notes">[ 💾 SALVAR ]</button>
          <button class="btn btn-danger" id="btn-clear-notes">[ LIMPAR ]</button>
          <span class="app-toolbar-sep"></span>
          <span style="font-family:var(--font-code);font-size:12px;color:var(--green-dark);">SALVO LOCALMENTE — APENAS NESTE DISPOSITIVO</span>
        </div>
        <textarea id="notepad-area" class="notepad-area" style="flex:1;resize:none;background:transparent;border:1px solid var(--border-dim);padding:12px;font-family:var(--font-code);font-size:15px;color:var(--green-mid);line-height:1.7;outline:none;" placeholder=">>> Suas anotações aqui...">${saved}</textarea>
      </div>`;
    }
    ...
    function initNotepad() {
        ...
        $('btn-save-notes')?.addEventListener('click', () => {
            localStorage.setItem(key, $('notepad-area')?.value || '');
    ...
    return {
        ...
        updateMissionStatus, changeUserRole, deleteUser
    };
